...
export function PaymentPage() {
  ...
  const handleCopy = () => {
    ...
    showToast(t('payment_copied_toast'));
  };
  ...
  const handleConfirmPayment = () => {
    ...
    saveOrder(orderData);
    showToast(t('payment_confirm_success'));
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="min-h-screen bg-gray-50 pb-24">
      <header className="sticky top-0 z-30 bg-white border-b border-gray-100 py-4 px-4 md:px-8">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <div className="flex items-center">
            ...
            <span className="font-serif text-xl font-medium ml-2">{t('payment_title')}</span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <div className="w-6 h-6 rounded-full bg-brand-dark text-white flex items-center justify-center text-[10px] font-bold">2</div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 flex flex-col gap-6">
        <div className="bg-[#FFF4E5] text-[#B26B00] px-4 py-3 rounded-lg flex items-center gap-2 text-sm font-medium">
          <Clock size={18} />
          {t('payment_complete_in')} {formatTime(timeLeft)}
        </div>
        ...
        <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
          {paymentMethod === 'bca' && (
            <>
              <div className="flex items-center gap-3 mb-6">
                <div className="px-3 py-1.5 bg-[#0066AE] text-white text-xs font-bold rounded">BCA</div>
                <div>
                  <h3 className="font-bold text-brand-dark">{t('payment_scan_qr').includes('QRIS') ? 'Virtual Account' : 'Virtual Account'}</h3>
                  <p className="text-xs text-gray-500">{t('payment_copy_va_desc')}</p>
                </div>
              </div>
              <div className="mb-6">
                <span className="text-xs text-gray-500 block mb-2">{t('payment_va_number')}</span>
                <div className="bg-gray-50 p-4 rounded-lg flex items-center justify-between">
                  <span className="font-mono text-xl md:text-2xl font-bold tracking-wider text-brand-dark">00816 95014 60982</span>
              ...
              <div className="bg-[#EAF5FD] p-4 rounded-lg">
                <h4 className="text-sm font-medium text-[#0066AE] mb-2">{t('payment_how_to')}</h4>
                <ol className="text-sm text-[#0066AE] space-y-1.5">
                  <li>{t('payment_bca_step1')}</li>
                  <li>{t('payment_bca_step2')}</li>
                  <li>{t('payment_bca_step3')}</li>
                  <li>{t('payment_bca_step4')}</li>
                </ol>
              </div>
            </>
          )}
          ...
          {paymentMethod === 'mandiri' && (
            <>
              <div className="flex items-center gap-3 mb-6">
                <div className="px-3 py-1.5 bg-[#003D79] text-white text-xs font-bold rounded">MDR</div>
                <div>
              ...
              <div className="mb-6">
                <span className="text-xs text-gray-500 block mb-2">{t('payment_va_number')}</span>
                <div className="bg-gray-50 p-4 rounded-lg flex items-center justify-between">
                  <span className="font-mono text-xl md:text-2xl font-bold tracking-wider text-brand-dark">89508 00816 95014</span>
                  <button onClick={handleCopy} className="px-4 py-2 bg-brand-dark text-white text-xs font-medium rounded-lg hover:bg-brand-accent transition-colors">
                    {copied ? t('payment_copied') : t('payment_copy')}
                  </button>
                </div>
              ...
              <div className="bg-[#EAF5FD] p-4 rounded-lg">
                <h4 className="text-sm font-medium text-[#003D79] mb-2">{t('payment_how_to')}</h4>
                <ol className="text-sm text-[#003D79] space-y-1.5">
                  <li>{t('payment_mandiri_step1')}</li>
                  <li>{t('payment_mandiri_step2')}</li>
                  <li>{t('payment_mandiri_step3')}</li>
                  <li>{t('payment_mandiri_step4')}</li>
                </ol>
              </div>
            </>
          )}
          ...
          {paymentMethod === 'qris' && (
            <>
              <div className="flex items-center gap-3 mb-6">
                <div className="px-3 py-1.5 bg-[#ED1C24] text-white text-xs font-bold rounded">QRIS</div>
                <div>
                  <h3 className="font-bold text-brand-dark">{t('payment_scan_qr')}</h3>
                  <p className="text-xs text-gray-500">{t('payment_scan_desc')}</p>
                </div>
              </div>
              ...
              <div className="bg-[#FEF2F2] p-4 rounded-lg">
                <h4 className="text-sm font-medium text-[#DC2626] mb-2">{t('payment_how_to')}</h4>
                <ol className="text-sm text-[#DC2626] space-y-1.5">
                  <li>{t('payment_qris_step1')}</li>
                  <li>{t('payment_qris_step2')}</li>
                  <li>{t('payment_qris_step3')}</li>
                  <li>{t('payment_qris_step4')}</li>
                </ol>
              </div>
            </>
          )}
