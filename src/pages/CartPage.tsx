...
const handleClose = () => {
  const state = location.state as { fromLooks?: boolean; fromModal?: string } | null;
  if (state?.fromLooks) {
    navigate('/', { state: { openLooks: true } });
  } else if (state?.fromModal === 'allProducts') {
              ...
              return (
                <>
                  {Object.entries(bundleGroups).map(([bundleName, indices]) => (
                    <div key={bundleName} className="border border-brand-accent/30 rounded-xl overflow-hidden">
                  ...
                  {standaloneIndices.map((idx) => renderItem(cartItems[idx], idx))}
                </>
              );
            })()}
          </div>
