...
export function Navbar({
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-40 transition-all duration-500 ease-in-out ${isScrolled ? 'bg-white/90 backdrop-blur-md border-b border-gray-100 py-4 shadow-sm' : 'bg-transparent py-6'}`}>  {/* ← ditambahin > */}
      ...
      <div className="max-w-[1440px] mx-auto px-4 md:px-8 flex items-center justify-between">
        <div className="flex items-center gap-2 md:gap-0">
          ...
          {/* Logo */}
            ...
            href="#"
            className={`font-serif text-2xl md:text-3xl font-medium tracking-wide transition-colors duration-500 ease-in-out ${isScrolled ? 'text-brand-dark' : 'text-brand-dark md:text-white'}`}>  {/* ← ditambahin > */}
            Ninetynine
          </a>
        </div>
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          <div
            ...
            <button
              className={`flex items-center gap-1 text-xs font-medium tracking-[0.15em] uppercase transition-colors duration-500 ease-in-out ${isScrolled ? 'text-brand-dark' : 'text-white'}`}>
              Category
              <ChevronDown
                size={14}
            ...
            href="#stores"
            className={`text-xs font-medium tracking-[0.15em] uppercase transition-colors duration-500 ease-in-out ${isScrolled ? 'text-brand-dark hover:text-brand-accent' : 'text-white hover:text-brand-neutral2'}`}>  {/* ← ditambahin > */}
            Offline Store
          </a>
        </nav>
        ...
        {/* Icons */}
        <div
          className={`flex items-center gap-4 md:gap-6 transition-colors duration-500 ease-in-out ${isScrolled || isSearchOpen ? 'text-brand-dark' : 'text-brand-dark md:text-white'}`}>  {/* ← ditambahin > */}
