      ...
      duration: 0.3
          ...
          <div className="max-w-[1440px] mx-auto px-4 md:px-12 py-10 md:py-16">
            ...
            {wishlist.length === 0 ? <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              </div> : <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-x-4 gap-y-10">
                {wishlist.map((product) => <ProductCard key={product.id} product={product} isWishlisted={true} onToggleWishlist={onToggleWishlist} onAddToCart={onAddToCart} />)}
              </div>}
          </div>
        </motion.div>}
    </AnimatePresence>;
